...
os.environ.update(
    LLM_MODEL="test-model", LLM_API_KEY="test-only",
    LLM_BASE_URL="https://example.com/v1", LLM_TIMEOUT="30",
)
...
class ToolCallTests(unittest.TestCase):
    def test_round_trip(self):
        client = MagicMock()
        client.chat.completions.create.side_effect = [response(calls=[call()]), response("485")]
        ...
        self.assertEqual(request["messages"][2], {
            "role": "tool", "tool_call_id": "call_1", "content": '{"result":485.0}',
        })
    ...
    def test_invalid_calls_never_execute(self):
        for calls in ([call(name="unknown")], [call(arguments='{"a":"1","b":2}')],
                      [call(arguments='not json')], [call(arguments='{"a":true,"b":2}')],
                      [call(arguments='{"a":1,"b":2,"extra":3}')],
                      [call(arguments='{"a":1e200,"b":2}')], [call(), call(id="call_2")]):
            with self.subTest(calls=calls), patch('hello_agent.sdk.p00_basics.tool_call.add') as add:
                client = MagicMock()
    ...
    def test_no_third_request(self):
        client = MagicMock()
        client.chat.completions.create.side_effect = [response(calls=[call()]), response(calls=[call()])]
