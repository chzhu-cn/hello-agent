...
from hello_agent.schemas.cooperation import CooperationResult, Delegation, ToolRecord, WorkerResult
...
def calculate(client: OpenAI, task: str, delegation: Delegation) -> WorkerResult:
    messages = [
        ...
        {"role": "user", "content": json.dumps({
            "original_task": task, "delegation": delegation.model_dump(),
        }, ensure_ascii=False)},
    ]
    records = []
    requests = 0
    try:
        for step in range(1, cooperation_config.worker_max_steps + 1):
            requests += 1
            logger.info("[计算者] 模型请求 {}/{}", step, cooperation_config.worker_max_steps)
            message = request_message(client, messages, tools=[ADD_TOOL])
            if not message.tool_calls:
                if not records:
                    raise ValueError("计算者没有执行工具，不能报告计算完成。")
                return WorkerResult(
                    status="completed", answer=message.content, records=records, model_requests=requests,
                )
            ...
            try:
                record = ToolRecord(arguments=arguments, result=add(arguments.a, arguments.b))
            except Exception:
                raise ValueError("工具执行失败，不自动重跑；保留已有记录。") from None
            records.append(record)
            logger.info("[计算者] add({}, {}) = {}", arguments.a, arguments.b, record.result)
            messages.extend([
        ...
        error = f"模型请求失败：{type(exc).__name__}" if isinstance(exc, APIError) else str(exc)
        logger.warning("[计算者] 返回失败：{}", error)
        return WorkerResult(status="failed", records=records, error=error, model_requests=requests)
...
def run(client: OpenAI) -> CooperationResult:
    ...
    try:
        ...
        if worker.status == "failed":
            ...
            coordinator.extend([
                ...
                {"role": "user", "content": "计算者返回：" + worker.model_dump_json()},
                {"role": "system", "content": (
    ...
    result = CooperationResult(
        status="failed" if error else "completed", answer=answer, error=error,
        delegation=delegation, worker=worker, coordinator_requests=requests,
        total_requests=requests + (worker.model_requests if worker else 0),
        elapsed_seconds=perf_counter() - started,
    )
    logger.info("停止状态：{}；协调者 {} 次，计算者 {} 次，总计 {} 次请求，工具 {} 次，耗时 {:.2f} 秒。",
                result.status, requests, worker.model_requests if worker else 0,
                result.total_requests, len(worker.records) if worker else 0, result.elapsed_seconds)
    if error:
