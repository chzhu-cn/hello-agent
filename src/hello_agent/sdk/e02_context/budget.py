...
def main() -> None:
    ...
    parser.add_argument("--preview", action="store_true", help="只查看预算和请求，不调用模型")
    args = parser.parse_args()
    history = example_history()
    try:
        original = build_messages(history, QUESTION, SYSTEM_PROMPT, ContextPolicy(mode="full"))
        messages = budget_messages(history, QUESTION, SYSTEM_PROMPT, budget_config)
        logger.info(
            "本地估算：原始 {}，裁剪后 {}；输入预算 {} = 总预算 {} - 输出预留 {} - 余量 {}。",
            estimate_tokens(original), estimate_tokens(messages),
            budget_config.context_tokens - budget_config.output_tokens - budget_config.safety_tokens,
            budget_config.context_tokens, budget_config.output_tokens, budget_config.safety_tokens,
        )
        logger.info("保留 {} 轮，移除 {} 轮；此估算不是模型精确 token 数。", (len(messages) - 2) // 2, (len(original) - len(messages)) // 2)
        ...
        with OpenAI(
            ...
            answer = request_text(client, messages, max_tokens=budget_config.output_tokens)
            logger.success("预算策略回复：{}", answer)
    except (APIError, ValueError) as exc:
        logger.error("预算或请求失败（{}）：{}；不重试。", type(exc).__name__, str(exc) if isinstance(exc, ValueError) else "服务请求失败")
