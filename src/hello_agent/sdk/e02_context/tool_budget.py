...
def example_history() -> ToolHistory:
    return ToolHistory.model_validate({"turns": [
        {"user": "分别计算 2+3 和 4+5。", "exchanges": [{
            "calls": [
                {"id": "add_1", "function": {"name": "add", "arguments": '{"a":2,"b":3}'}},
                {"id": "add_2", "function": {"name": "add", "arguments": '{"a":4,"b":5}'}},
            ],
            "results": [
...
def main() -> None:
    ...
    try:
        ...
        messages = budget_tool_history(
            example_history(), prompt,
            system_prompt, budget_config,
        )
        logger.info("预置历史不执行工具；整轮裁剪后本地估算 {}，消息 {} 条。", estimate_tool_tokens(messages), len(messages))
        ...
        with OpenAI(
            ...
            logger.success("回复：{}", request_text(client, messages, max_tokens=budget_config.output_tokens))
