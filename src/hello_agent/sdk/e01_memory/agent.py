...
def persistent_chat(client: OpenAI, session: Conversation, prompt: str, name: str) -> Conversation:
...
def main() -> None:
    ...
    parser.add_argument("--session", help="保存或恢复指定会话（字母、数字、下划线、连字符，最多 64 字符）")
    ...
    try:
        with OpenAI(
            ...
            if not args.interactive:
                ...
                return
            name = args.session
            session = load_session(memory_config.directory, name) if name else Conversation()
            logger.info("输入内容后回车；/new 新建会话，/exit 退出。")
            if name:
                logger.info("会话 {} 已加载 {} 条历史；每轮成功后保存到磁盘。", name, len(session.messages))
            else:
            ...
            while True:
                ...
                try:
                    ...
                    logger.error("磁盘保存失败（{}），本轮未提交到当前会话，不自动重试。", type(exc).__name__)
