...
def run(client: OpenAI, prompt: str) -> str:
    ...
    requests += 1
    answer = request_text(client, [
        {"role": "system", "content": "根据原始任务、计划及真实执行结果用中文回答。没有工具结果时不要声称执行过工具；不支持的任务应说明限制。"},
        {"role": "user", "content": prompt},
    ...
    logger.success("最终回答：\n{}", answer)
    logger.info("正常完成：模型请求 {} 次，工具执行 {} 次，耗时 {:.2f} 秒。", requests, len(records), perf_counter() - started)
    return answer


def main() -> None:
    parser = argparse.ArgumentParser(description="体验先规划再执行")
    parser.add_argument("prompt", nargs="?", default="先计算 127 加 358，再将前一步结果加上 96。")
