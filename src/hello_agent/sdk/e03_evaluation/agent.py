...
def cases() -> list[EvaluationCase]:
    ...
    examples = [
        ("记住颜色", ["我最喜欢青绿色。"], "青绿色"),
        ("更新偏好", ["我最喜欢红色。", "现在我最喜欢蓝色，之前的偏好不再适用。"], "蓝色"),
        ("新会话隔离", [], "不知道"),
        ("无关历史", ["今天练习加法。"], "不知道"),
    ...
    return [EvaluationCase(
        name=name, question=question, expected=expected,
        history=Conversation(messages=[
            message for text in texts for message in (
...
def evaluate(client: OpenAI, items: list[EvaluationCase]) -> list[EvaluationResult]:
    results = []
    for case in items:
        print(f"案例 {case.name}：system={SYSTEM_PROMPT}；历史={case.history.model_dump_json()}；问题={case.question}")
        ...
        result = EvaluationResult(
            name=case.name, expected=case.expected, answer=answer,
            completed=answer is not None, passed=answer is not None and grade(answer, case.expected),
            error=error, requests=1, elapsed_seconds=round(perf_counter() - started, 3),
        )
        results.append(result)
        logger.info("评估结果：{}", result.model_dump_json())
    logger.info(
        "汇总：共 {} 题，完成 {}，规则通过 {}，请求尝试 {}，工具执行 0。",
        len(results), sum(r.completed for r in results), sum(r.passed for r in results), sum(r.requests for r in results),
    )
    return results
...
def main() -> None:
    if llm_config.base_url is None:
        ...
        raise SystemExit(1)
    logger.info("E03-A 固定案例 v1；模型 {}；每题一次请求，无工具，无自动重试。", llm_config.model)
    with OpenAI(
