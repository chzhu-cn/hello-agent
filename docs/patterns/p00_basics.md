...
当前状态：学习说明已编写，模型服务尚未确定，代码与实验尚未开始。
...
具体调用方法、参数名称和文本提取方式在选型后依据对应官方文档确定，本阶段不预设某家服务的接口格式。
...
| 模型服务 | 确定使用哪家的服务 | 待定 |
| 模型名称 | 指定处理请求的模型 | 待定 |
| 模型 SDK | 确定使用哪个 Python 客户端库 | 待定 |
| 认证信息 | 让服务识别调用者及其权限；常见方式是 API Key | 根据服务确定，密钥不写入代码、日志或 Git |
| 服务地址 | 指定 API 入口；部分 SDK 提供默认地址 | 按 SDK 要求确定 |
| 请求超时 | 防止程序无期限等待 | 实现前确定 |
...
这条输入足够短，容易判断回复是否相关，也无需外部工具或实时数据。初版建议把它作为固定输入，先观察完整调用过程；交互方式仍需确认。
...
P0 的工具调用部分会在下一学习步骤补充。当前下一步是确定模型服务、模型、SDK 和初期交互方式。
